...
const FlatList_Item = (props) => {
    const { question, options, correct, questionId, category, BookmarkInstantiation, UpvoteInstantiation, DownvoteInstantiation } = props
    ...
    const handleBookmarkPress = async (questionId) => {
        ...
        if (isBookmarked) {
            ...
            const response = await fetch(`http://172.31.33.189/software_project/remove_bookmark`, {   //Ansh =>172.31.33.189, Jay => 172.31.33.189
                method: "post",
                headers: {
...
export default function User_helper_Three({ route, navigation }) {
    const { downvoted_questions, questions, upvoted_questions, bookmarked_questions } = route.params.data
